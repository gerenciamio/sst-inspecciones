...
def exportar_excel(visita_id):
    ...
    def apply(cell, val, bold=False, sz=10, h='center', va='center', fill=None, border=False):
        cell.value = val
    ...
    for i, hh in enumerate([14,14,33,33,35], 1):
        ws1.row_dimensions[i].height = hh
    apply(ws1['A1'], 'LOGO', bold=True)
    apply(ws1['C1'], 'SEGURIDAD SALUD EN EL TRABAJO', bold=True)
    apply(ws1['F1'], f'Fecha: {v["fecha"]}', bold=True, sz=8)
    apply(ws1['C2'], 'INFORME Y SEGUIMIENTO A INSPECCIONES', bold=True)
    apply(ws1['F2'], 'CODIGO: FT-SST-020', bold=True, sz=8)
    apply(ws1['F3'], 'Pag 1 de 2', bold=True, sz=8)
    apply(ws1['G3'], 'Version 1', bold=True)
    ...
    for i, h in enumerate(hs):
        ...
        apply(ws1[f'A{r}'], h.get('lugar',''), border=True, h='left', va='top')
        apply(ws1[f'B{r}'], h.get('situacion',''), border=True, h='left', va='top')
        apply(ws1[f'G{r}'], h.get('recomendacion',''), border=True, h='left', va='top')
        estado = h.get('estado','pendiente')
        apply(ws1[f'H{r}'], 'Cerrado' if estado=='cerrado' else 'Pendiente',
              border=True, fill='C0DD97' if estado=='cerrado' else 'FAC775')
        apply(ws1[f'E{r}'], '', border=True)
        apply(ws1[f'F{r}'], '', border=True)
    ...
    ws2['A1'].value = 'MATRIZ DE SEGUIMIENTO A LOS PLANES DE ACCION'
